...
{% hint style="info" %}
If you still rely on the old behvior of the new line splitter, you can use the `SplitNewLinesOld()` function. Beware that it will not split strings that contain only carriage return as a new line (mostly strings that are made by [Mac OS 9](https://en.m.wikipedia.org/wiki/Mac_OS_9)).
...
* `RandomizeArray()`: Uses the [Schwartzian transform](http://en.wikipedia.org/wiki/Schwartzian_transform) to randomly sort the array.
