...
11. You can make your subs anywhere on the class, but if: i) they're on the different class, press Enter on End Class and make a public new class that stores new subs:
...
ii) they're trying to re-initialize the screensaver by re-calling InitSaver(), Try so on your test environment first, then the Production environment if that worked properly.

* And the following conditions should be met: i) They shouldn't make an infinite loop unless you're making them that exits if specified conditions are met ii) They shouldn't try to cause errors with the kernel. iii) Put your sub call on one of the three subs that implements the ICustomSaver interface. Ex. If you're going to make a sub that's going to be called on screensaver display, place your sub call on the ScrnSaver() sub, and then your code on your sub.

12. Run the build. When the build is successful, ignore the dialog box that appears.
13. Run your Kernel Simulator you've just referenced to in your project, and load, set default, and lock your screen and your screensaver is there.
...
```vb
    ...
    Dim BackMessages As String() = {"<OWNER> will be back soon.", "<OWNER> is busy. He will be back soon."}
        ...
        W("   | {0} |", False, ColTypes.Neutral, text)
        InitializeBar(text.Length)
    End Sub
```
...
```vb
    ...
    Public Class SoonOvernightRude
        ...
        Public R_Random As New Random() 'Initializes the random number generator
        Sub InitSaver() Implements ICustomSaver.InitSaver
            W("Set this screensaver as default using ""setsaver SORSS""", False, ColTypes.Neutral)
            SaverName = "SORSS"
            Initialized = True
        End Sub
        Sub PreDisplay() Implements ICustomSaver.PreDisplay
    ...
    Public Sub ScrnSaver() Implements ICustomSaver.ScrnSaver
        For CurrentStep = 0 To 3
            ...
            If Custom.CancellationPending = True Then 'This will fix the issue for the task being busy.
                Exit For
            End If
            W(aniset(CurrentStep), True, ColTypes.Neutral)
            Thread.Sleep(150)
        Next
    End Sub
...
Public Class Dynamic
    ...
    Public Sub ShowStats()
        W("{0} / 1000000 | {1}%", False, ColTypes.Neutral, Res, FormatNumber(CStr(Res * 100 / 1000000), 1))
        UpdateProgressBar(Res * 100 / 1000000)
    End Sub
    Public Sub UpdateProgressBar(ByVal percent As String)
        Check = percent / 5
        W(Check, False, ColTypes.Neutral)
