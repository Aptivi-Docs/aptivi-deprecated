...
iii) IMPORTANT NOTICE! Never try to use infinite loops in handlers unless you're making infinite loops that exits if the specified condition is met, or in the test environment. That will lock the kernel up. 25. You can make your subs anywhere on the class, but if: i) they're on the different class, press Enter on End Class and make a public new class that stores new subs:
